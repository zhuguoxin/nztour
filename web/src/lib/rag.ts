...
/**
 * Build a Claude system prompt that bakes in the citation context.
 * Output asks Claude to cite using [^1], [^2] markers we can re-map client-side.
 */
export function buildSystemPrompt(citations: Citation[]): string {
  ...
  return `You are TourTrain's product assistant for New Zealand travel agents.
...
You have access to ${citations.length} relevant snippet${citations.length === 1 ? "" : "s"} from the operators' own course content. When the answer is grounded in a snippet, cite it inline using bracketed markers like [^1], [^2]. Do NOT cite snippets that don't actually support your statement.
...
Now answer the agent's question. Keep it under 200 words unless a longer answer is genuinely needed.`;
}
