...
export function BillingForm({
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-5 max-w-md space-y-4">
      <div>
        <div className="text-caption font-medium text-slate-600 mb-1">{tr.bill_plan_label}</div>
        <div className="inline-block px-2.5 py-1 rounded-md bg-emerald-50 border border-emerald-200 text-slate-900 text-caption font-mono uppercase">
          {planTier}
        </div>
        <div className="text-caption text-slate-500 mt-1.5">{tr.bill_plan_note}</div>
      </div>

      <form action={formAction} className="space-y-2">
        <input type="hidden" name="supplier_slug" value={supplierSlug} />
        <label className="block">
          <span className="text-caption font-medium text-slate-600 mb-1 block">{tr.bill_email_label}</span>
        ...
        <div className="flex items-center gap-2 pt-1">
          <button
            type="submit"
            disabled={pending}
            className="px-3.5 py-1.5 rounded-md bg-emerald-600 text-white font-semibold text-small hover:bg-emerald-700 disabled:opacity-50"
          >
      ...
      <div className="text-caption text-slate-500 border-t border-slate-200 pt-3">{tr.bill_invoices_soon}</div>
    </div>
  );
}
