  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white">
      <header className="px-5 py-4 border-b border-slate-200">
        <div className="font-semibold text-small text-slate-900">{tr.tp_title}</div>
        ...
        {TRANSLATE_LANGS.map((lang) => {
          ...
          if (isPrimary) {
            return (
              <span
                key={lang.code}
                className="px-3 py-1.5 rounded-md bg-emerald-600 text-white font-semibold text-caption"
                title={tr.tp_source}
              >
                {lang.label} · {tr.tp_source}
              </span>
            );
          }
            ...
            return (
              <div key={lang.code} className="flex items-stretch gap-1">
                <button
                  ...
                  disabled={pending}
                  className="px-3 py-1.5 rounded-md border border-emerald-400/50 bg-emerald-600/10 text-slate-900 hover:bg-emerald-600/15 text-caption font-medium disabled:opacity-50"
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="px-2 py-1.5 rounded-md border border-slate-300 text-slate-600 hover:bg-slate-50 text-micro"
                ...
                <button
                  ...
                  disabled={pending}
                  className="px-2 py-1.5 rounded-md border border-slate-300 text-slate-600 hover:bg-slate-50 text-micro disabled:opacity-40"
          ...
          if (isTranslated) {
            return (
              <button
                ...
                disabled={pending}
                className="px-3 py-1.5 rounded-md border border-slate-300 text-slate-500 hover:bg-slate-50 hover:text-slate-700 text-caption disabled:opacity-40"
          ...
          return (
            <button
              ...
              disabled={pending}
              className="px-3 py-1.5 rounded-md border border-slate-300 text-slate-600 hover:bg-slate-50 hover:text-slate-900 hover:border-emerald-400/40 text-caption disabled:opacity-40"
      ...
      {pending && busyLang ? (
        <div className="px-5 pb-4 text-caption text-slate-900 flex items-center gap-2">
