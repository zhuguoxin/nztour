...
export function CourseSwitcher({
  ...
  return (
    <div>
      <div className="text-[11px] tracking-widest font-mono text-emerald-700/70 mb-1.5">{label}</div>
      <select
        value={current}
        onChange={(e) => router.push(`/product/${operatorSlug}/courses/${e.target.value}/edit`)}
        className="w-full bg-white border border-slate-300 rounded-md px-3 py-2 text-[13.5px] text-slate-900 outline-none focus:border-emerald-400/60"
      >
        {courses.map((c) => (
