...
/**
 * Real "upload content → generate course" panel. Picks a PDF/text file and
 * POSTs it to /api/course/generate (Claude drafts a structured course), then
 * opens the new course's editor. "More options" links to the full new-course
 * screen (title/notes hints + manual mode).
 *
 * `bare` renders just the dropzone + actions (no card chrome) so it can sit
 * inside another card — e.g. the "My courses" card footer.
 */
export function CreateCoursePanel({ operatorSlug, bare = false }: { operatorSlug: string; bare?: boolean }) {
  const tr = useTr();
