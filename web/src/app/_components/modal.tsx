      ...
      >
        {title !== undefined ? (
          <div className="flex items-center justify-between px-5 py-3.5 border-b border-slate-200 sticky top-0 bg-white rounded-t-2xl">
            <div className="font-semibold text-[15px] text-slate-900">{title}</div>
            <button
              type="button"
              onClick={onClose}
              aria-label="Close"
              className="w-8 h-8 rounded-md text-slate-500 hover:bg-slate-100 flex items-center justify-center text-[18px] leading-none"
            >
              ✕
            </button>
          </div>
